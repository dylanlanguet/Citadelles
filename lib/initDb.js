 ...
 * Crée les tables pour les BuildingCards et les CharacterCards.
 * Insert des exemples de BuildingCards et de CharacterCards si la table est vide.
...
async function initDb() {
  ...
  await db.exec(`
      CREATE TABLE IF NOT EXISTS BuildingCards (
        id INTEGER PRIMARY KEY,            -- ID unique de la carte
        title TEXT NOT NULL,               -- Titre de la carte
        content TEXT NOT NULL,             -- Description ou contenu de la carte
        type TEXT NOT NULL,                -- Type de la carte (par ex. commercial, militaire)
        cost INTEGER NOT NULL,             -- Coût en ressources pour jouer la carte
        utility TEXT,                      -- Pouvoir ou effet spécial de la carte
        quantity INTEGER NOT NULL          -- Quantité totale de cette carte dans le jeu
      );
    `);
  ...
  await db.exec(`
      CREATE TABLE IF NOT EXISTS CharacterCards (
        id INTEGER PRIMARY KEY,            -- ID unique du personnage
        title TEXT NOT NULL,               -- Nom du personnage
        content TEXT NOT NULL,             -- Description du personnage
        type TEXT NOT NULL,                -- Type de personnage (par ex. assassin, marchand)
        power TEXT NOT NULL                -- Pouvoir unique du personnage
      );
    `);
  ...
  if (buildingCardsCount.count === 0) {
    const insertStmt = `
      ...
      (122, 'Donjon', 'Une ancienne tour imprenable aux murs épais.', 'Merveille', 3, 'Le Donjon ne peut pas être détruit par le Condottière.', 2),
